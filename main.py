...
def start(message):
	...
	if f == SUDO:
		...
		M = types.InlineKeyboardButton('DEV', url='https://t.me/W_I_X')
		mas.add(G,K)
		mas.add(V)
		mas.add(M)
		...
		me = types.InlineKeyboardButton(text="خيال",url="https://t.me/W_I_X")
		he = types.InlineKeyboardButton(text="خيال",url="https://t.me/W_I_X")
		de = types.InlineKeyboardButton(text="خيال",url="https://t.me/W_I_X")
		ch = types.InlineKeyboardButton(text="▶ المطور ◀",url="https://t.me/W_I_X")
		rr.add(me,he,de)
		rr.add(ch)
...
def masg(call):
	...
	if call.data =="W_I_X":
		...
		while True:
			...
			W_I_X = [h1,h2,h3,h4,h5]
			j = random.choice(W_I_X)
			url = "https://t.me/"+j
			headers = {
            "User-Agent": generate_user_agent(),
            "Accept": "text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,image/apng,*/*;q=0.8",
			...
			if response.text.find('If you have <strong>Telegram</strong>, you can contact <a class="tgme_username_link"')>=0 and j not in ban:
				...
				R = types.InlineKeyboardButton(f'{sk}', callback_data="1x")
				W_I_X = types.InlineKeyboardButton('DEV', url='https://t.me/W_I_X')
				mas.add(A,E,B,R,W_I_X)
				bot.edit_message_text(chat_id=call.message.chat.id,message_id=call.message.message_id,text="ok start",reply_markup=mas)
